...
export default function Pathlight() {
  ...
  const parseSynthesis = (content) => {
    ...
    try {
      ...
      // Extract superpowers section - AI uses ### headers for each superpower
      const superpowersSection = content.match(/## Your Superpowers\n\n([\s\S]*?)(?=\n## Your Deal-Breakers|$)/);
      if (superpowersSection) {
        const fullText = superpowersSection[1];
        
        // Split on ### headers (superpower names)
        const powerBlocks = fullText.split(/\n### /);
        
        powerBlocks.forEach((block, blockIdx) => {
          if (!block.trim()) return;
          
          const lines = block.split('\n');
          
          // First line is the name (with ** around it)
          ...
          // Skip if this is "Why these matter" or "Your unique value"
          if (name.includes('Why these matter') || name.includes('Your unique value')) return;
          
          console.log(`Processing superpower: "${name}"`);
          
          // Second non-empty line after name is the description
          let description = '';
          let descStart = 1;
          while (descStart < lines.length && !lines[descStart].trim()) descStart++;
          if (descStart < lines.length) {
            // Description is everything until we hit "*How to deploy this:*"
            let descLines = [];
          ...
          // Extract "How to deploy this" section - everything after "*How to deploy this:*" until next ### or "**Why these matter"
          const howToStart = block.indexOf('*How to deploy this:*');
          if (howToStart !== -1) {
            let howToEnd = block.length;
            
            // Find where this section ends - either at "**Why these matter" or end of block
            ...
            console.log(`  How-to length: ${howToText.length}`);
        ...
        // Extract "Why these matter together"
    ...
    console.log('Superpowers found:', sections.superpowers.length);
    sections.superpowers.forEach((p, i) => {
      console.log(`Power ${i + 1}:`, p.name);
      console.log('Description:', p.description.substring(0, 100) + '...');
    });
    console.log('Summary:', sections.superpowersSummary);
  ...
  const loadProgress = async () => {
    try {
      const result = await window.storage.get('pathlight-data');
      if (result?.value) {
  ...
  const saveProgress = async (newConversations, newSection, newAiCount, newScreener, newSynthesis) => {
    try {
      await window.storage.set('pathlight-data', JSON.stringify({
  ...
  const resetProgress = async () => {
    if (confirm('Start a fresh journey? All your progress will be reset.')) {
      try {
        await window.storage.delete('pathlight-data');
        setConversations({});
        setScreenerAnswers({});
        setCurrentSection(0);
        setAiResponseCount({});
...
`;
      ...
      SECTIONS.forEach(s => {
        if (conversations[s.id] && s.type !== 'intro' && s.type !== 'screener' && s.type !== 'synthesis') {
          allData += `\n${s.title}:\n`;
                            ...
                            <details className="group">
                              ...
                              <div className="mt-4 pl-4 text-gray-700 text-sm leading-relaxed space-y-3">
                                {power.howTo.split('\n').map((line, idx) => {
                                  ...
                                  if (bulletMatch) {
                                    const label = bulletMatch[1].replace(/:$/, ''); // Remove trailing colon from label
                                    return (
                  ...
                  <div className="grid gap-6">
                    {parsedData.dealbreakers.work.length > 0 && (
                      <div className="bg-white rounded-3xl p-6 shadow-lg border-l-4 border-red-400">
                        <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center gap-2">
                          <span className="text-red-500">❌</span> About the work
                        </h3>
                    ...
                    {parsedData.dealbreakers.job.length > 0 && (
                      <div className="bg-white rounded-3xl p-6 shadow-lg border-l-4 border-orange-400">
                        <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center gap-2">
                          <span className="text-orange-500">❌</span> What comes with the job
                        </h3>
                        <ul className="space-y-2 text-gray-700 text-sm">
                    ...
                    {parsedData.dealbreakers.leadership.length > 0 && (
                      <div className="bg-white rounded-3xl p-6 shadow-lg border-l-4 border-amber-400">
                        <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center gap-2">
                          <span className="text-amber-500">❌</span> About leadership
                        </h3>
                        <ul className="space-y-2 text-gray-700 text-sm">
                    ...
                    {parsedData.dealbreakers.environment.length > 0 && (
                      <div className="bg-white rounded-3xl p-6 shadow-lg border-l-4 border-yellow-400">
                        <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center gap-2">
                          <span className="text-yellow-500">❌</span> About the environment
                        </h3>
                        <ul className="space-y-2 text-gray-700 text-sm">
