...
const S = {
  ...
  bodyLg: { fontFamily: "'Inter', sans-serif", fontSize: '1.0625rem', fontWeight: 300, lineHeight: 1.7, color: '#A8A4A0' },
  body: { fontFamily: "'Inter', sans-serif", fontSize: '0.9375rem', fontWeight: 300, lineHeight: 1.65, color: '#A8A4A0' },
  small: { fontFamily: "'Inter', sans-serif", fontSize: '0.8125rem', fontWeight: 300, lineHeight: 1.5, color: '#5C5955' },
};
...
export default function Pathlight() {
  ...
  useEffect(() => {
    try {
      ...
      if (p.get('payment')==='success'||p.get('unlock')==='true') { setPaymentCompleted(true); localStorage.setItem(PAYMENT_KEY,'done'); window.history.replaceState({},'',window.location.pathname); }
...
SHORT responses. Questions, not advice.`;
  ...
  if (screen==='landing') return (<><Fonts/><Dark center><Orb s={600} o={0.12}/><Grid/><div style={{textAlign:'center',zIndex:1}}><h1 style={{...S.display,fontSize:'clamp(3.5rem,10vw,7rem)',fontWeight:800,letterSpacing:'-0.04em',animation:'fadeIn 1.2s ease forwards'}}>Pathlight</h1><p style={{...S.bodyLg,marginTop:24,opacity:0,animation:'fadeIn 0.8s ease 0.4s forwards'}}>Find clarity in your career</p><div style={{marginTop:48,opacity:0,animation:'fadeIn 0.8s ease 0.8s forwards'}}><button onClick={()=>{setScreen('intro');setIntroPage(0);}} style={{display:'inline-flex',alignItems:'center',gap:8,padding:'12px 28px',background:'#F0EDE8',color:'#0A0A0B',...S.small,fontWeight:500,border:'none',borderRadius:4,cursor:'pointer'}}>Begin <ArrowRight size={16}/></button></div><p style={{...S.mono,color:'#5C5955',marginTop:16,opacity:0,animation:'fadeIn 0.8s ease 1.2s forwards'}}>20 minutes · private · ai-powered</p></div></Dark></>);

  if (screen==='intro'&&introPage===0) return (<><Fonts/><Dark center><Grid o={0.015}/><div style={{maxWidth:560,width:'100%',zIndex:1,animation:'screenFadeIn 0.5s ease forwards'}}><MonoLabel>before we start</MonoLabel><DisplayH>You're here because something isn't quite right.</DisplayH><div style={{display:'flex',flexDirection:'column',gap:20,marginTop:32}}>{ ['Maybe you feel stuck in a role that doesn\'t fit anymore.','Maybe you lack direction.','Maybe you\'re ready for something bigger but don\'t know what.'].map((t,i)=><p key={i} style={{...S.bodyLg,opacity:0,animation:`fadeIn 0.6s ease ${0.2+i*0.15}s forwards`}}>{t}</p>)}</div><p style={{...S.bodyLg,color:'#F0EDE8',marginTop:32}}>That's okay. You're in the right place.</p><div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:24,marginTop:32}}><p style={S.body}>This works like a conversation with a sharp friend who sees your blind spots. The more honest and specific you are, the more useful your report will be. Give yourself 20 minutes somewhere quiet.</p></div><div style={{marginTop:48,display:'flex',justifyContent:'flex-end'}}><button onClick={()=>setIntroPage(1)} style={{display:'inline-flex',alignItems:'center',gap:8,padding:'12px 28px',background:'#F0EDE8',color:'#0A0A0B',...S.small,fontWeight:500,border:'none',borderRadius:4,cursor:'pointer'}}>Continue <ArrowRight size={16}/></button></div></div></Dark></>);

  if (screen==='intro'&&introPage===1) return (<><Fonts/><Dark center><Grid o={0.015}/><div style={{maxWidth:560,width:'100%',zIndex:1,animation:'screenFadeIn 0.5s ease forwards'}}><MonoLabel>after one conversation</MonoLabel><DisplayH>What you'll walk away with</DisplayH>{['Five signature strengths — uniquely named and actionable','Deal-breakers you should never compromise on','Role territories that fit how you actually work','A career narrative ready for LinkedIn'].map((t,i)=><div key={i} style={{display:'flex',alignItems:'flex-start',gap:16,padding:'20px 0',borderBottom:i<3?'1px solid rgba(255,255,255,0.04)':'none'}}><span style={{...S.mono,color:'#B8A9FF',minWidth:24,paddingTop:4}}>0{i+1}</span><p style={S.bodyLg}>{t}</p></div>)}<div style={{marginTop:48,display:'flex',justifyContent:'space-between',alignItems:'center'}}><button onClick={()=>setIntroPage(0)} style={{display:'inline-flex',alignItems:'center',gap:6,background:'none',border:'none',color:'#5C5955',...S.small,cursor:'pointer'}}><ArrowLeft size={14}/> Back</button><button onClick={()=>setScreen('consent')} style={{display:'inline-flex',alignItems:'center',gap:8,padding:'12px 28px',background:'#F0EDE8',color:'#0A0A0B',...S.small,fontWeight:500,border:'none',borderRadius:4,cursor:'pointer'}}>Let's start <ArrowRight size={16}/></button></div></div></Dark></>);

  if (screen==='consent'){const ok=consentChecks.terms&&consentChecks.age&&consentChecks.data;return(<><Fonts/><Dark center><Grid o={0.015}/><div style={{maxWidth:520,width:'100%',zIndex:1,animation:'screenFadeIn 0.5s ease forwards'}}><div style={{display:'flex',alignItems:'center',gap:12,marginBottom:32}}><Lock size={20} style={{color:'#5C5955'}}/><div><div style={{...S.subhead}}>Before we begin</div><div style={{...S.mono,color:'#5C5955'}}>your privacy is protected</div></div></div><div style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:24,marginBottom:32}}><p style={S.body}>Your data stays on your device. Responses are processed by AI to generate insights. We don't store your information on our servers.</p></div>{[{key:'terms',text:'I agree to the Terms of Service and understand this is a self-reflection tool, not professional career advice'},{key:'age',text:'I am 18 years of age or older'},{key:'data',text:'I understand my responses will be processed by AI'}].map(item=><div key={item.key} onClick={()=>setConsentChecks(p=>({...p,[item.key]:!p[item.key]}))} style={{display:'flex',alignItems:'flex-start',gap:16,cursor:'pointer',padding:16,borderRadius:8,border:`1px solid ${consentChecks[item.key]?'rgba(184,169,255,0.3)':'rgba(255,255,255,0.06)'}`,background:consentChecks[item.key]?'rgba(184,169,255,0.06)':'#111113',transition:'all 150ms',marginBottom:8}}><div style={{width:20,height:20,minWidth:20,borderRadius:4,marginTop:2,border:`2px solid ${consentChecks[item.key]?'#B8A9FF':'rgba(255,255,255,0.1)'}`,background:consentChecks[item.key]?'#B8A9FF':'transparent',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 150ms'}}>{consentChecks[item.key]&&<Check size={12} color="#0A0A0B"/>}</div><span style={S.body}>{item.text}</span></div>)}<button onClick={ok?startChat:undefined} disabled={!ok} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'14px 36px',marginTop:32,background:ok?'#B8A9FF':'#1A1A1D',color:ok?'#0A0A0B':'#3A3835',...S.body,fontWeight:500,border:'none',borderRadius:4,cursor:ok?'pointer':'not-allowed',transition:'all 350ms'}}>{ok?'Start the conversation':'Please agree to all items'}</button><div style={{textAlign:'center',marginTop:16,...S.mono,color:'#3A3835'}}><Link href="/privacy" style={{color:'#5C5955'}}>privacy</Link>{' · '}<Link href="/terms" style={{color:'#5C5955'}}>terms</Link></div></div></Dark></>);}

  if (screen==='chat') return (<><Fonts/><div style={{minHeight:'100vh',background:'#F5F5F3',display:'flex',flexDirection:'column'}}><div style={{padding:'16px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',background:'#F5F5F3',position:'sticky',top:0,zIndex:10}}><span style={{fontFamily:"'Syne',sans-serif",fontSize:'1rem',fontWeight:700,letterSpacing:'-0.02em',color:'#111113'}}>Pathlight</span><span style={{...S.mono,color:'#555250',display:'flex',alignItems:'center',gap:6}}><Lock size={12}/> private</span></div><div style={{height:2,background:'#E5E3DF',margin:'0 24px'}}><div style={{height:'100%',background:'#7B6FBF',width:`${Math.min((userMsgCount/15)*100,100)}%`,transition:'width 500ms ease',borderRadius:1}}/></div><div style={{flex:1,overflowY:'auto',padding:'24px 16px'}}><div style={{maxWidth:640,margin:'0 auto',display:'flex',flexDirection:'column',gap:12}}>{parsedMessages.map((msg,idx)=><div key={idx} style={{alignSelf:msg.role==='user'?'flex-end':'flex-start',maxWidth:'85%',padding:'14px 18px',borderRadius:8,background:msg.role==='user'?'#111113':'#FFFFFF',color:msg.role==='user'?'#F5F5F3':'#111113',border:msg.role==='user'?'none':'1px solid #E5E3DF',fontFamily:"'Inter',sans-serif",fontSize:'0.9375rem',fontWeight:400,lineHeight:1.65,whiteSpace:'pre-wrap'}}>{msg.text}</div>)}{isLoading&&<div style={{alignSelf:'flex-start',padding:'14px 18px',background:'#FFFFFF',border:'1px solid #E5E3DF',borderRadius:8,display:'flex',gap:4}}><span className="pl-dot"/><span className="pl-dot" style={{animationDelay:'0.2s'}}/><span className="pl-dot" style={{animationDelay:'0.4s'}}/></div>}<div ref={messagesEndRef}/></div></div><div style={{background:'#F5F5F3',padding:16}}><div style={{maxWidth:640,margin:'0 auto'}}>{chatComplete?<div style={{textAlign:'center'}}><button onClick={()=>{setScreen('synthesis');generateSynthesis();}} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'14px 28px',background:'#111113',color:'#F5F5F3',...S.small,fontWeight:500,border:'none',borderRadius:4,cursor:'pointer'}}>Generate My Report <ArrowRight size={16}/></button><p style={{...S.mono,color:'#555250',marginTop:8}}>takes about 45 seconds</p></div>:activeInteraction?<InteractionWidget interaction={activeInteraction} onSubmit={handleInteractionSubmit}/>:<div style={{display:'flex',gap:12}}><input ref={inputRef} type="text" value={userInput} onChange={e=>setUserInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleTextSubmit();}}} placeholder="Type your response..." disabled={isLoading} style={{flex:1,padding:'12px 16px',background:'#F5F5F3',border:'1px solid #E5E3DF',borderRadius:4,fontFamily:"'Inter',sans-serif",fontSize:'0.9375rem',color:'#111113',outline:'none'}}/><button onClick={handleTextSubmit} disabled={isLoading||!userInput.trim()} style={{padding:'12px 16px',background:'#111113',color:'#F5F5F3',border:'none',borderRadius:4,cursor:'pointer',display:'flex',alignItems:'center',opacity:(!userInput.trim()||isLoading)?0.2:1}}><Send size={18}/></button></div>}</div></div><div style={{padding:'8px 24px',display:'flex',justifyContent:'space-between',background:'#F5F5F3',...S.mono,color:'#555250',fontSize:'0.625rem'}}><div style={{display:'flex',gap:12}}><Link href="/privacy" style={{color:'#555250'}}>privacy</Link><Link href="/terms" style={{color:'#555250'}}>terms</Link></div><button onClick={resetProgress} style={{background:'none',border:'none',color:'#555250',...S.mono,fontSize:'0.625rem',cursor:'pointer'}}>clear data</button></div></div></>);
